--[[
    ...
    Version: 2.0.6 (Stability & Performance)
    Author: Ferocious
    Description: Monitor de consumíveis e buffs para Midnight com foco em performance.
]]
...
local lastUpdate = 0
local UPDATE_INTERVAL = 1.5 -- Intervalo otimizado para não pesar na CPU
...
local FLASK_IDS_WEAK = { [1235057]=true, [1235058]=true, [1235059]=true, [1235060]=true }
local FLASK_IDS_STRONG = { [1230874]=true, [1230857]=true, [1235061]=true, [435422]=true, [443210]=true }
local OIL_IDS = { [8051]=true, [8052]=true, [8053]=true, [8054]=true } -- EnchantIDs
local RUNE_IDS = { [1235065]=true, [1264426]=true, [1270500]=true }
local FOOD_IDS_WEAK = { [1232317]=true, [1232318]=true, [1284616]=true }
local FOOD_IDS_STRONG = { [1233709]=true, [1233716]=true, [1247001]=true }
...
local function ScanUnitBuffs(unit, reqClassBuffs)
    local statusFO, statusFL, hasOL, hasR, hasB = 0, 0, false, false, true
    local foundBuffs = {}

    -- Uso de loop dinâmico para performance máxima
    local i = 1
    while true do
    ...
    -- Proteção de API: GetWeaponEnchantInfo só funciona no 'player'
    if not hasOL and unit == "player" then
        local hasMain, _, _, mainEnchantID = GetWeaponEnchantInfo()
        if hasMain and OIL_IDS[mainEnchantID] then hasOL = true end
    end

    if reqClassBuffs then
        for sid in pairs(reqClassBuffs) do
...
local function ReportBuffsToChat()
    ...
    for _, u in ipairs(units) do
        if UnitExists(u) then
            local sFO, sFL, hOL, hR, hB = ScanUnitBuffs(u, req)
            local name = UnitName(u)
            if sFO == 0 then table.insert(noFood, name) end
            if sFL == 0 then table.insert(noFlask, name) end
            if not hOL and u == "player" then table.insert(noOil, name) end
...
local function CreatePlayerLine(index)
    ...
    for i, tag in ipairs(tags) do
        ...
        txt:SetPoint("RIGHT", -5 - (5-i)*21, 0); txt:SetText(tag)
...
local function UpdateGroupBuffs()
    ...
    for _, unit in ipairs(units) do
        if UnitExists(unit) then
            local sFO, sFL, hOL, hR, hB = ScanUnitBuffs(unit, req)
...
events:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then inCombat = false; if BuffCheckDB.visible then mainFrame:Show() end end
end)
